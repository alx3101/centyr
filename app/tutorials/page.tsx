...
import { Upload, Palette, Ruler, Plug, Package, Video, Clapperboard, ArrowLeft, ArrowRight } from 'lucide-react'
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-fuchsia-50 py-16 px-4">
      <div className="max-w-3xl mx-auto">
        ...
        {/* Coming soon banner */}
        <div className="bg-amber-50 border-2 border-amber-200 rounded-xl p-4 mb-8 text-sm text-amber-800 text-center flex items-center justify-center gap-2">
          <Clapperboard className="w-4 h-4 shrink-0" /><span>{it
            ? 'I video tutorial sono in produzione e saranno disponibili a breve. Nel frattempo consulta le guide scritte qui sotto.'
            : 'Video tutorials are in production and coming soon. In the meantime, check out the written guides below.'}</span>
        </div>
        ...
        <div className="mt-8 bg-white/80 backdrop-blur-sm border-2 border-purple-100 rounded-2xl shadow-xl p-6 text-center">
          ...
          >
            <span className="inline-flex items-center gap-1">{it ? 'Centro Assistenza' : 'Help Center'} <ArrowRight className="w-4 h-4" /></span>
          </Link>
        </div>
